...
class HttpClient {
  ...
  Future<String> download({
    ...
    try {
      ...
      dio.options.headers = headers ?? this.headers;
      dio.interceptors.add(
        LogInterceptor(
          ...
          error: enableLogs,
          logPrint: (object) => _log(object, 'download', enableLogs),
        ),
      );
      await dio.download(
        url,
      ...
      _log('Downloaded file from $url to $savePath', 'download', enableLogs);
      return savePath;
    } on DioException catch (e) {
      _log(e, 'download', enableLogs);
      throw HttpException('Download failed: ${e.message}');
    } finally {
      dio?.close();
    }
  }
