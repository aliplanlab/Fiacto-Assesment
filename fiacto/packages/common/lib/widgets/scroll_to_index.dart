...
mixin AutoScrollControllerMixin on ScrollController
  ...
  Future<dynamic> _scrollToIndex(
    ...
    Future<dynamic> makeSureStateIsReady() async {
    ...
    if (isIndexStateInLayoutRange(index)) {
      ...
      bool contains = false;
      Duration spentDuration = Duration.zero;
  ...
  Future<dynamic> _waitForWidgetStateBuild() =>
  ...
  Future<dynamic> _bringIntoViewportIfNeed(
    int index,
    AutoScrollPosition? preferPosition,
    Future<dynamic> Function(double offset) move,
  ) async {
  ...
  double _directionalOffsetToRevealInViewport(int index, double alignment) {
    ...
    if (tagOffsetInViewport == null) {
      ...
      final double absoluteOffsetToViewport = tagOffsetInViewport.offset;
      if (alignment == 0.5) {
        return absoluteOffsetToViewport;
      } else if (alignment == 0) {
...
class AutoScrollTag extends StatefulWidget {
  ...
  @override
  AutoScrollTagState createState() => AutoScrollTagState<AutoScrollTag>();
}
...
class AutoScrollTagState<W extends AutoScrollTag> extends State<W>
  ...
  Future<dynamic> highlight({
    ...
    await Future<void>.delayed(highlightDuration);
...
Future<T> co<T>(Object key, FutureOr<T> Function() action) async {
  for (;;) {
    final c = _locks[key];
    if (c == null) break;
    try {
...
final _locks = HashMap<dynamic, Completer<dynamic>>();

/// skip the TickerCanceled exception
Future<dynamic> catchAnimationCancel(TickerFuture future) async {
